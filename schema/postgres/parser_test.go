...
func TestCrossSchemaFK(t *testing.T) {
	s := mustParse(t, `
	...
	if products.Schema != "inventory" {
		t.Errorf("products schema = %q, want inventory", products.Schema)
	}

	orders := findTable(s, "orders")
	if orders == nil {
	...
	if fk.RefTable != "products" {
		t.Errorf("FK RefTable = %q, want products", fk.RefTable)
	}
	if fk.RefSchema != "inventory" {
		t.Errorf("FK RefSchema = %q, want inventory", fk.RefSchema)
	}
	if fk.OnDelete != schema.ActionRestrict {
		t.Errorf("FK OnDelete = %q, want RESTRICT", fk.OnDelete)
	}
}

func TestCompositeFKMultiColumn(t *testing.T) {
	s := mustParse(t, `
		CREATE TABLE locations (
			region TEXT NOT NULL,
			zone TEXT NOT NULL,
			name TEXT NOT NULL,
			PRIMARY KEY (region, zone)
		);
		CREATE TABLE events (
			id SERIAL PRIMARY KEY,
			loc_region TEXT NOT NULL,
			loc_zone TEXT NOT NULL,
			description TEXT,
			CONSTRAINT fk_event_loc FOREIGN KEY (loc_region, loc_zone) REFERENCES locations(region, zone) ON DELETE CASCADE ON UPDATE SET NULL
		);
	`)

	events := findTable(s, "events")
	if events == nil {
		t.Fatal("events table not found")
	}
	if len(events.ForeignKeys) != 1 {
		t.Fatalf("expected 1 FK, got %d", len(events.ForeignKeys))
	}

	fk := events.ForeignKeys[0]
	if fk.Name != "fk_event_loc" {
		t.Errorf("FK name = %q, want fk_event_loc", fk.Name)
	}
	if len(fk.Columns) != 2 || fk.Columns[0] != "loc_region" || fk.Columns[1] != "loc_zone" {
		t.Errorf("FK Columns = %v, want [loc_region loc_zone]", fk.Columns)
	}
	if len(fk.RefColumns) != 2 || fk.RefColumns[0] != "region" || fk.RefColumns[1] != "zone" {
		t.Errorf("FK RefColumns = %v, want [region zone]", fk.RefColumns)
	}
	if fk.OnDelete != schema.ActionCascade {
		t.Errorf("FK OnDelete = %q, want CASCADE", fk.OnDelete)
	}
...
func TestIdentityAlwaysAsIdentity(t *testing.T) {
	s := mustParse(t, `
		CREATE TABLE sequences (
			id INTEGER GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
			seq_by_default INTEGER GENERATED BY DEFAULT AS IDENTITY,
			name TEXT NOT NULL
		);
	`)

	table := findTable(s, "sequences")
	if table == nil {
		t.Fatal("sequences table not found")
	}

	id := table.FindColumn("id")
	if !id.IsAutoIncrement {
		t.Error("id should be auto-increment (GENERATED ALWAYS AS IDENTITY)")
	}
	if !id.HasDefault {
		t.Error("id should have default")
	}
	if id.IsNullable {
		t.Error("id (PK) should not be nullable")
	}

	seq := table.FindColumn("seq_by_default")
	if !seq.IsAutoIncrement {
		t.Error("seq_by_default should be auto-increment (GENERATED BY DEFAULT AS IDENTITY)")
	}
	if !seq.HasDefault {
		t.Error("seq_by_default should have default")
	}

	name := table.FindColumn("name")
	if name.IsAutoIncrement {
		t.Error("name should not be auto-increment")
	}
}
...
func TestArrayColumns(t *testing.T) {
	...
	table := findTable(s, "array_test")
	if table == nil {
		t.Fatal("array_test table not found")
	}
	...
	tests := []struct {
		...
		{"int_arr", true, 1, true},
		{"text_arr", true, 2, false},
		{"bool_arr", true, 1, true},
		{"json_arr", true, 1, true},
	}
	...
	for _, tt := range tests {
		...
		if col.IsArray != tt.isArray {
			t.Errorf("col %q: IsArray = %v, want %v", tt.name, col.IsArray, tt.isArray)
		}
		if col.ArrayDims != tt.dims {
			t.Errorf("col %q: ArrayDims = %d, want %d", tt.name, col.ArrayDims, tt.dims)
		}
		if col.IsNullable != tt.nullable {
...
func TestMultiIndex(t *testing.T) {
	s := mustParse(t, `
		...
		CREATE UNIQUE INDEX idx_products_name_cat ON products (name, category_id);
	`)
	...
	if len(products.Indexes) != 4 {
		t.Fatalf("expected 4 indexes, got %d", len(products.Indexes))
	}

	// First index: unique on sku
	idx0 := products.Indexes[0]
	if idx0.Name != "idx_products_sku" || !idx0.Unique {
		t.Errorf("index 0: name=%q unique=%v, want idx_products_sku/true", idx0.Name, idx0.Unique)
	}
	if len(idx0.Columns) != 1 || idx0.Columns[0] != "sku" {
		t.Errorf("index 0 columns = %v, want [sku]", idx0.Columns)
	}

	// Second index: non-unique on category_id
	idx1 := products.Indexes[1]
	if idx1.Name != "idx_products_category" || idx1.Unique {
		t.Errorf("index 1: name=%q unique=%v, want idx_products_category/false", idx1.Name, idx1.Unique)
	}

	// Third index: non-unique on name
	idx2 := products.Indexes[2]
	if idx2.Name != "idx_products_name" || idx2.Unique {
		t.Errorf("index 2: name=%q unique=%v, want idx_products_name/false", idx2.Name, idx2.Unique)
	}

	// Fourth index: unique composite on (name, category_id)
	idx3 := products.Indexes[3]
	if idx3.Name != "idx_products_name_cat" || !idx3.Unique {
		t.Errorf("index 3: name=%q unique=%v, want idx_products_name_cat/true", idx3.Name, idx3.Unique)
	}
	if len(idx3.Columns) != 2 {
		t.Errorf("index 3 columns = %v, want 2 columns", idx3.Columns)
	}

	// Unique indexes should also produce UniqueConstraints.
	uniqueCount := 0
	for _, u := range products.Unique {
		if u.Name == "idx_products_sku" || u.Name == "idx_products_name_cat" {
			uniqueCount++
		}
	}
	if uniqueCount != 2 {
		t.Errorf("expected 2 unique constraints from unique indexes, got %d", uniqueCount)
	}
}
