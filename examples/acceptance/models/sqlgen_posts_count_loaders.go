...
func init() {
	...
	postCountLoaders["Comments"] = loadCountPostComments
}
...
func loadCountPostComments(ctx context.Context, exec sqlgen.Executor, d sqlgen.Dialect, parentModels any) error {
	models := parentModels.(PostSlice)
	if len(models) == 0 {
		return nil
	}
	...
	counts, err := sqlgen.LoadPolymorphicCount(ctx, exec, d, "comments", "commentable_type", "Post", "commentable_id", ids)
	if err != nil {
		return err
	}
