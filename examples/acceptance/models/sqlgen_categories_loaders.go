...
func init() {
	...
	categoryLoaders["Posts"] = loadCategoryPosts
}
...
func loadCategoryPosts(ctx context.Context, exec sqlgen.Executor, d sqlgen.Dialect, parentModels any, loads []*sqlgen.EagerLoadRequest) error {
	models := parentModels.(CategorySlice)
	if len(models) == 0 {
		return nil
	}
	...
	mods := categoryLoadMods(loads, "Posts")
	rows, err := sqlgen.LoadMany(ctx, exec, d, PostTableName, "category_id", ids, mods...)
	if err != nil {
		return err
	}
	if rows == nil {
