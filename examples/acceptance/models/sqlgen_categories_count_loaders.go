...
func init() {
	...
	categoryCountLoaders["Posts"] = loadCountCategoryPosts
}
...
func loadCountCategoryPosts(ctx context.Context, exec sqlgen.Executor, d sqlgen.Dialect, parentModels any) error {
	models := parentModels.(CategorySlice)
	if len(models) == 0 {
		return nil
	}
	...
	counts, err := sqlgen.LoadCount(ctx, exec, d, "posts", "category_id", ids)
	if err != nil {
		return err
	}
