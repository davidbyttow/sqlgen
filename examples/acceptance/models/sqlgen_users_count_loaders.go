...
func init() {
	...
	userCountLoaders["Comments"] = loadCountUserComments
}
...
func loadCountUserComments(ctx context.Context, exec sqlgen.Executor, d sqlgen.Dialect, parentModels any) error {
	models := parentModels.(UserSlice)
	if len(models) == 0 {
		return nil
	}
	...
	counts, err := sqlgen.LoadCount(ctx, exec, d, "comments", "author_id", ids)
	if err != nil {
		return err
	}
